...
const Invoices = ({
    ...
    const [consolidatedShouldRefresh, setConsolidatedShouldRefresh] = useState(false);

    const [customer, setCustomer] = useState(0);
    const [consolidateCustomer, setConsolidateCustomer] = useState(0);
    const [consolidateableInvoices, setConsolidateableInvoices] = React.useState<InvoicesType[]>([]);
    ...
    function handleInvoiceSelect(item: InvoicesType) {
        const newConsolidated = consolidateableInvoices.map((_item) => {
            if (_item.ID === item.ID) {
                _item.Consolidated = _item.Consolidated !== 1 ? 1 : 0;
            }
            return _item;
        });
    ...
    trpc.useQuery(["invoices.getAllUnpaid", {
        ...
        <Grid2 container>
            <Grid2 xs={8} sx={{paddingRight: 2.5}}>
                <Box sx={{
                    ...
                    <Button type={"button"}
                            variant={"contained"}
                            style={{backgroundColor: "#FFA726", alignSelf: "flex-end", height: 32}}
                            onClick={() => {
                                toggleModal(true)
                            }}
                    >
            ...
            >
                <Box sx={style}>
                    <div style={{display: "flex", flexDirection: "column"}}>
                        <b style={{textAlign: "center"}}>Select Unpaid Invoices to Consolidate</b>
                        <div
                            style={{
                                paddingBottom: 5,
                                display: "flex",
                    ...
                    >
                        <Button
                            ...
                            onClick={async () => {
                                await createConsolidated.mutateAsync({ids: consolidateableInvoices.filter((item) => item.Consolidated).map((item) => item.ID)});
