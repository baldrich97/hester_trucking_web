...
const TableRow = ({load, invoice = null}: {load?: CompleteLoads, invoice?: CompleteInvoices | null}) => (
    <View style={styles.container}>
        {invoice !== null ? <>
            <Text style={{width: '8%', ...styles.leftAlignNoPadding, ...styles.text}}>{new Date(invoice.InvoiceDate).toLocaleDateString()}</Text>
            ...
            <Text style={{width: '8%', ...styles.leftAlignNoPadding, ...styles.text, fontSize: 9}}>{new Date(load.StartDate).toLocaleDateString()}</Text>
            <Text style={{width: '10%', textAlign: 'left', ...styles.padding, ...styles.text}}>{load.Trucks?.Notes ? load.Trucks?.Notes.split('#').length > 1 ? load.Trucks?.Notes.split('#')[1] : load.Trucks?.Notes.split('#')[0] : 'N/A'}</Text>
            <Text style={{width: '25%', textAlign: 'left', ...styles.padding, ...styles.text}}>{load.LoadTypes?.Description ?? 'N/A'}</Text>
            <Text style={{width: '28%', textAlign: 'left', ...styles.padding, ...styles.text}}>{load.DeliveryLocations?.Description ?? 'N/A'}</Text>
            ...
            <Text style={{width: '10%', textAlign: 'right', ...styles.text, paddingRight: 5}}>${load.TotalAmount ? (Math.round(load.TotalAmount * 100) / 100).toString() : 'N/A'}</Text>
        </> : null}
    </View>
)
